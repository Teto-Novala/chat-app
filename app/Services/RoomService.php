...
class RoomService
{
        ...
        if ($lastMessage) {
            ...
            if ($lastMessage->sender_id == $user->id) {
                $statusData = $lastMessage->statusForOthers->map(function ($status) {
                    return [
                        'recivier' => $status->recipient_id,
                        'status' => $status->status,
                        'read_at' => $status->read_at ? $status->read_at->toDateTimeString() : null,
                    ];
                });
            } else {
                $totalUnread = $room->messages()->whereHas('statuses', function ($query) use ($user) {
                    $query->where('recipient_id', $user->id)->where('status', 'delivered');
                })->count();
            }
    ...
    {
        return User::where('name', 'like', '%' . $search . '%')->where('id', '!=', Auth::id())->get()->map(function ($user) {
            return [
                'id' => $user->id,
                'type' => 'contact',
                'user_id' => $user->id,
